...
公开 v0.14.7 已于用户“好的请上线发布”授权后发布。39 个线上文件 HTTP 200 且逐字节匹配发布提交；线上实际预览、玩家／家长分别换装、刷新保存、进入游戏与暂停检查通过，浏览器无错误／警告。见 [线上核验记录](online-validation.json)；以下模型和本地检查属于建模轮（日志 60）。
