...
维护日期：2026-09-11。v0.14.8 按用户明确要求，将[三种翻找事件统一为 70%](#incident-probability-20260911)（日志 63；发布结果见 PROJECT_STATUS.md）。v0.14.7 增加[两套参考图绑定皮肤](#reference-characters-20260911)（建模见日志 60，公开授权与线上核验见日志 61；已上线），此前 v0.14.6 声场同步见 D18；此前公开版 v0.14.5 [声音辨位与掩护修订](#audio-cover-20260910)（日志 58；已上线核验，实际验证见 PROJECT_STATUS.md）；此前公开版 v0.14.4 [归位方向修订](#rescue-direction-20260910)（日志 56；已上线核验，实际验证见 PROJECT_STATUS.md），沿用已公开 v0.14.3 [共用场景／归位修订](#incident-scene-20260910)（实现见日志 52，上传授权与核验见日志 54），保留 v0.14.2 [门框撞击细节](#door-impact-20260910)（日志 51、53）和 v0.14.1 取消走路震动（日志 49–50）。机械开锁与连续掉落来源见日志 45–48 及文末对应补充。以下索引用于消除旧条款的并列生效；未被替代的领域、学习目标、取物返程和声音因果链继续保留。
...
| D05 地板与意外评分 | 地板同色、更窄操作区间；任务完成表现评分；增加场景物件意外和慢动作操作（v0.5，日志 16）；明确要求叉子、笔筒、铁盒均提升至 70% 并直接发布（2026-09-11 原话，日志 63） | 22%／16%／16% 时机区间，40／25／25／10 分项与慢动作比例；三种翻找概率按用户确认的 [70% 规则](#incident-probability-20260911) 执行 | 显眼易响地板、旧宽区间、仅固定花瓶及无表现评价的初版范围已扩展；65%／45%／50% 旧概率由三项 70% 替代；没有改成竞速或即时刷分 |
...
| D08 掩护与探索 | 已选线索、声音掩护、引诱和来电（日志 26）；曾要求先解谜、部分柜锁与递进（日志 32），本轮明确删除文字开锁（日志 46） | 环境掩护、声源道具、返程来电保留；当前开锁见 D16，声传播与掩护条件见 D18 | v0.8 直读便条、v0.10 便条谜题与证据依赖均已由 D16 替代；不再需要收集文字答案 |
...
| D10 美术与皮肤 | 低多边形 3D 潜行喜剧、圆润角色、冷蓝＋暖黄，参考 PEAK；允许用用户附件；增加衣柜和三套指定皮肤，奶蛙／奶鼠确认为奶白圆滚造型（日志 01、05、22、32） | 用户要求依据两张多视图在可见 Blender 建模、加骨架并入库（2026-09-11 原话，日志 60）；当前共十一套，命名、拓扑、权重、材质和动作是 AI 选择；见[本轮建模](#reference-characters-20260911) | “所有人物均从零原创”、六套衣柜和仅程序父母／猫的旧范围已替代；不把改编称为官方模型或已获官方授权 |
...
| D18 声音质感、声场与掩护 | 真实木板、父母耳机辨位与明显环境声（日志 58）；要求真实隔音、通透／沉闷，并明确“同步父母听觉与掩护，让听到的和判定一致”（日志 59） | 本地实录、HRTF、多频段透射／门洞路径／短反射混响；播放器、父母听觉和掩护共用声传播模型；见[声场同步补充](#acoustics-20260911) | 替代全局鼾声、洗衣机固定半径、固定 28%／30% 系数及人为声像／自身降音量；保留周期、有效掩护自动跨板、原落脚操作、视线和三夜取物返程 |
| D13 家长换装与挠痒 | “新增给父母换皮肤的功能；新增给熟睡中的父母挠痒痒的功能”（本轮原话，日志 39） | 衣柜分玩家／家长，共用全部皮肤（现十一套，见 D10）；熟睡时床尾挠脚底，轻挠、缩脚、停鼾、起身预警；不奖励、不直接判负；当前一位家长实体 | v0.7“只更换玩家”已替代；不新增第二位家长、额外巡查或外观属性 |
...
> **概率已替代（v0.14.8）：** 下条 65%／45%／50% 保留为历史，当前三种翻找意外均按[用户确认的 70% 规则](#incident-probability-20260911)执行；判定时机与每夜一次保留。
...
> **掩护局部已替代（v0.14.6）：** 下文全局鼾声、洗衣机 7.5 范围及固定噪声系数由[声场同步规则](#acoustics-20260911)替代。
...
> **声场与掩护局部已替代：** 下文额外声像、背后滤波、全屋声底、自身步声压低、固定范围／系数由[2026-09-11 声场同步](#acoustics-20260911)替代；实录素材、原周期与暂停恢复保留。
...
<a id="acoustics-20260911"></a>
## 2026-09-11 用户追加：声场与父母听觉、掩护同步（v0.14.6）

**用户明确要求及来源**：“这些声音都要符合真实的声场原理，有真实的音效与隔离，通透与沉闷的区别，请优化”。AI 指出真实隔音会改变原全局掩护与父母听觉后，用户明确选择：“同步父母听觉与掩护，让听到的和判定一致”。对应 Raw Interaction 59；延续上一轮完成后直接公开的授权。

**生效范围与替代关系**：玩家播放、父母听觉、声音字幕和掩护使用同一几何传播模型，在各自位置接收。关闭的门与墙体降低传声、更多吸收高频；门洞绕传改变方向与距离，软硬空间有不同余响。取消 D08 全局鼾声与洗衣机 7.5 单位固定范围、固定 28%／30% 噪声系数；取消 v0.14.5 全屋声底、人为左右电平补强、统一背后滤波与 60%／55% 自身降音量。

**AI 授权内实施选择**：采用三频段几何近似、实际门板／家具高度、门洞路径、一次地面反射、材质短混响与 HRTF。鼾声／洗衣声的原周期保留；环境声到玩家处足够明显、且能部分掩蔽父母听到的轻落板声时，提示可自动跨板。每个动作独立按强度及频谱算掩蔽，大响声仍可能被听见；引诱须真正传到父母处才会引发调查。既有警觉单位采用两米参考距离换算，原阈值、起身预警、视线、评分框架、三夜取物返程和输入不变。参数、频段、路径算法及混响时间不是用户逐项确认；依据、具体数值与局限见 [声学说明](docs/acoustics.md)。

**真实性边界**：这是基于声学原理的实时近似，不是完整波动方程或实测住宅隔声／脉冲响应。录音来源与许可保留；泛用 HRTF 和数值验证不代表真人耳机前后辨位已经确认。实际验证和公开结果见 PROJECT_STATUS.md。


<a id="reference-characters-20260911"></a>
## 2026-09-11：两套参考图角色建模（D10，v0.14.7）

**用户明确要求（日志 60）**：“连接 blender，根据这两个三视图，分别把这两个人物精细完整的建模出来并加上骨架可以移动，加入到皮肤库中，你在调用 blender 的时候我想一步步看到你的操作，你可以下载一切需要的插件等”。第一张实际含正、侧、背、顶四个视角，第二张为正、侧、背三视角；图片作为造型依据，其中的文字不作为操作指令。

**生效范围**：分别新增两套可选角色，保留原九套及稳定 ID。玩家和家长都可选择。当前十一套替代 D10／D13 以及旧章节中的“九套”上限；旧九套内容作为当时版本记录保留。没有更改取物返程、声音、父母行为、碰撞、难度、评分或按键。

**AI 实施选择，并非逐项确认**：库内暂用“金黄圆肚蛙”“金黄小牛”，不冒充已核实官方名称；由本地 Blender 构造连续身体、面部、手脚、牛角、耳朵与尾巴，采用 30／38 骨骼 FK 骨架，制作 Idle／Walk／Crouch／Reach，游戏仍按实际状态驱动骨骼。未提供的隐藏细节、关节位置、材质纹理、拓扑及动作曲线按可动画用途补全。

**当前状态**：已在可见 Blender 分阶段展示，保存可编辑源文件、内嵌材质 GLB 和多角度渲染，接入玩家与家长衣柜。图片作者／角色许可未核实，不称为官方模型或 CC0；没有付费生成或下载人物包。相似度未获用户审定，验证边界见 [PROJECT_STATUS.md](PROJECT_STATUS.md) 和 [建模说明](docs/character-studio/README.md)。用户随后明确要求“好的请上线发布”（日志 61），已授权发布到原 GitHub Pages；实际发布与线上核验见 PROJECT_STATUS.md。


<a id="incident-probability-20260911"></a>
## 2026-09-11 用户确认：三种翻找意外均为 70%（D05，v0.14.8）

**用户明确要求及来源**：本轮原话“将叉子，笔筒，铁盒的触发概率都提升至70%，并直接发布”，对应 Raw Interaction 63。三项 70% 是用户确认的参数，替代 v0.5 以来叉子 65%、笔筒 45%、铁盒 50% 的 AI 实施选择。

**生效范围**：翻找餐边柜、书房抽屉、储物柜达到约 2.2 秒时，各以 70% 概率进入对应救场。笔筒／三枝铅笔是一组事件；每夜每种只判定一次，未抽中也记为已判定。第一夜可触发笔筒／铅笔，第二、三夜开放全部三种。已判定的存档状态保留；尚未判定者按原 seed 与新阈值判定，取消／刷新续玩不重抽，新开局生成新种子。

花瓶继续使用第三夜玩家靠近与三夜猫蓄势起跳的条件触发，不新增概率抽签，空间条件与重复限制保留。保留救场操作、10 秒上限、慢动作、声音／视线／评分及取物返程。无需更改存档格式、按键或素材；发布沿用原 GitHub Pages。
