...
维护日期：2026-09-10。当前功能更新 v0.13，来源见日志 42 和文末本轮补充。以下索引用于消除旧条款的并列生效；未被替代的领域、学习目标、取物返程和声音因果链继续保留。
...
| D04 门轴 | “通过门轴转动的咯吱声音”自然引导调速，不直接显示速度条，尽量用真实素材（v0.11，日志 35） | E／鼠标渐进施力、R2 压深、角度涩点、连续实录与扶门特写；轻推没有最低速度惩罚 | 第 2、5、8 节及旧补充中的固定适宜门速区间、过慢罚声、途中反复撞门、滑块／滚轮／左右键／L1-R1 调速、按住 □ 推门已替代。取消最低速度惩罚是 AI 实施选择，并非用户原话 |
...
| D06 视角与翻找 | 先全景，移动后第一人称，小地图切全景；后续增加跟随近景第三人称和具体翻找特写（v0.2、v0.10，日志 05、32） | V／R3 切近景，M／△ 全景返回；抽屉和翻书有动画，翻找／接物用专用展示场景，开门用实际场景；读题暂停、翻找和开门世界继续 | 第 6、7 节“以高位第三人称为主的初版探索方案”“所有特写始终共用同一场景”等已被后续镜头实现替代；初版镜头条款不能触发回退 |
...
| D08 线索与探索 | 已选线索、声音掩护、引诱、返程来电方案；后续明确要求先解谜才看线索，部分柜子有锁，难度逐关上升（v0.8、v0.10，日志 26、32） | 每夜三处线索，图案／数字／倒序柜锁，证据依赖、提示、解锁后翻找与来电时长为 AI 设计 | v0.8 两张直接可读便条、仅可选帮助、无证据要求的搜索方式被 v0.10 替代；不恢复直给答案 |
...
| D12 关门与空间音效 | “现在的开门玩法我很喜欢，新增关门功能”；要求网上真实声音、不同材质脚步、听声辨位与远近（本轮原话，日志 39） | 沿用 D04，加入 R / L1 反向、防夹和半关续玩；三种地面四段实录、HRTF、距离和遮挡滤波、短时方向字幕 | 仅可开门的范围扩展为主动开合；D04 的无速度条与收力判断保留；程序脚步、简单左右平移被实录与空间处理替代 |
...
| D14 物件救场与手柄 | “完善并优化花瓶掉落，文具掉落等的玩法……结合手柄的功能想一些很有创意的”（本轮原话，日志 39） | 摇杆移手，双扳机托稳花瓶／压铁盒盖、袖口止响叉子、分路拦铅笔；最长 10 秒慢动作，键鼠对应操作；标准轻重震动 | 第 3–5、7 节和 v0.5 统一空格／× 指针接物在新事件中被替代；松动地板仍用原指针，旧存档中途接物兼容旧操作。保留预警、声音后果、原评分框架 |
| D15 PS5 触觉 | “再为 ps5 手柄增加自适应扳机功能以及震动功能”，列举开关门发涩轻颤、材质落脚、花瓶／铅笔不同接触脉冲，允许增加用途（本轮原话，日志 42） | USB WebHID 自适应阻力；门轴、双手托物、止响、挠痒；材质脚步、逐枝接笔、手机、摸猫等差异化震动，独立开关／强度与释放保护 | 扩展 D14 的标准震动；替代 v0.12“没有自适应扳机”的范围。具体曲线、USB 首发及额外用途为 AI 实施选择；不改变动作判定、难度或噪声 |
...
<!-- maintenance-note:start -->
> **线索规则已替代：** 两张可选直读便条被三处解谜线索和柜锁证据要求替代，见[索引](#current-rules) D08；掩护、引诱与来电规则保留。
<!-- maintenance-note:end -->
...
> **范围已扩展（v0.12）：** 在本节施力与门轴规则上新增关门、反向和防夹，见 D12；原开门手感继续保留。
