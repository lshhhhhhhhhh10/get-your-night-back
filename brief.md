...
维护日期：2026-09-11。v0.14.7 增加[两套参考图绑定皮肤](#reference-characters-20260911)（建模见日志 60，公开授权见日志 61；发布中），此前 v0.14.6 声场同步见 D18；此前公开版 v0.14.5 [声音辨位与掩护修订](#audio-cover-20260910)（日志 58；已上线核验，实际验证见 PROJECT_STATUS.md）；此前公开版 v0.14.4 [归位方向修订](#rescue-direction-20260910)（日志 56；已上线核验，实际验证见 PROJECT_STATUS.md），沿用已公开 v0.14.3 [共用场景／归位修订](#incident-scene-20260910)（实现见日志 52，上传授权与核验见日志 54），保留 v0.14.2 [门框撞击细节](#door-impact-20260910)（日志 51、53）和 v0.14.1 取消走路震动（日志 49–50）。机械开锁与连续掉落来源见日志 45–48 及文末对应补充。以下索引用于消除旧条款的并列生效；未被替代的领域、学习目标、取物返程和声音因果链继续保留。
...
**当前状态**：已在可见 Blender 分阶段展示，保存可编辑源文件、内嵌材质 GLB 和多角度渲染，接入本地衣柜。图片作者／角色许可未核实，不称为官方模型或 CC0；没有付费生成或下载人物包。相似度未获用户审定，验证边界见 [PROJECT_STATUS.md](PROJECT_STATUS.md) 和 [建模说明](docs/character-studio/README.md)。用户随后明确要求“好的请上线发布”（日志 61），已授权发布到原 GitHub Pages；实际发布与线上核验见 PROJECT_STATUS.md。
