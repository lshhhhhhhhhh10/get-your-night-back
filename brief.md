...
维护日期：2026-09-10。当前公开版 v0.14；门轴、机械开锁与掉落动画来源见日志 45–46 及文末对应补充，上传授权与线上核验见日志 47–48。以下索引用于消除旧条款的并列生效；未被替代的领域、学习目标、取物返程和声音因果链继续保留。
...
| D04 门轴 | 通过真实门轴声自然引导调速、不显示速度条（日志 35）；用户认可自适应扳机可用，要求慢速轻响／中速涩响／高速减响但易撞停（日志 45） | E／鼠标渐进施力、R2 压深、角度涩点、连续实录；摩擦随实际转速先增后减，开合终点高速撞击一次、提前收力可避免；细节见[本轮门轴修订](#door-physics-20260910) | 原固定正确区间、过慢罚声、途中反复撞门和速度条已替代；v0.11 随施力单调增响被本轮分段曲线替代；松手即停保留 |
...
| D06 视角与翻找 | 先全景，移动后第一人称，小地图切全景；后续增加跟随近景第三人称和具体翻找特写（v0.2、v0.10，日志 05、32） | V／R3 切近景，M／△ 全景返回；抽屉和翻书有动画，翻找／接物用专用展示场景，开门用实际场景；文字读题已由 D16 删除；开锁、翻找和开门世界继续 | 第 6、7 节“以高位第三人称为主的初版探索方案”“所有特写始终共用同一场景”等已被后续镜头实现替代；初版镜头条款不能触发回退 |
...
| D17 掉落动作风格 | “优化现在的花瓶掉落文具掉落的动画化，要有风格感，现在太粗糙”（本轮原话，日志 46） | 失衡预备、接触缓冲、花枝余摆、分路滚落、落地回弹与收回；陶瓶／花朵／铅笔重制，双段袖口手臂、投影与少量动作线；见同一补充 | D14 具体救场动作、事件概率、最长 10 秒、评分与慢动作因果保留；替代瞬移到结果、铅笔原地旋转及统一粗糙模型表现 |
