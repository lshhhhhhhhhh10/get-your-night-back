...
function buildHouse(){
  ...
  ceiling=box(maxX+1,.08,15,'#556279',center,2.58,7);ceiling.visible=false;ceiling.castShadow=false;
...
let audioContext,master,effectsGain,ambienceGain,muted=false;
function applyAudio(){if(master){master.gain.value=muted?0:settings.master;effectsGain.gain.value=settings.effects;ambienceGain.gain.value=settings.ambience*.09;}}
function enableAudio(){
  if(!audioContext){
    audioContext=new(window.AudioContext||window.webkitAudioContext)();master=audioContext.createGain();effectsGain=audioContext.createGain();ambienceGain=audioContext.createGain();effectsGain.connect(master);ambienceGain.connect(master);master.connect(audioContext.destination);
    const buffer=audioContext.createBuffer(1,audioContext.sampleRate*5,audioContext.sampleRate),d=buffer.getChannelData(0);let n=0;for(let i=0;i<d.length;i++){n=(n+(Math.random()*2-1)*.03)/1.02;d[i]=n;}
    const src=audioContext.createBufferSource();src.buffer=buffer;src.loop=true;const filter=audioContext.createBiquadFilter();filter.type='lowpass';filter.frequency.value=230;src.connect(filter);filter.connect(ambienceGain);src.start();applyAudio();
  }audioContext.resume().catch(()=>{});
}
function sound(kind,strength,x,z){
  if(!audioContext||muted)return;const now=audioContext.currentTime,g=audioContext.createGain(),pan=audioContext.createStereoPanner();
  // 声音左右方向跟随玩家正在看的方向。
  pan.pan.value=clamp(((x-game.player.x)*Math.cos(view.yaw)+(z-game.player.z)*Math.sin(view.yaw))/6,-.9,.9);g.connect(pan);pan.connect(effectsGain);
  const dist=Math.max(1,distance({x,z},game.player)),loud=Math.min(.19,.08+(strength||20)/450)/(1+dist*.11);
  if(['step','parentStep','crash','bed','snore','wobble','creak'].includes(kind)){
    const duration=kind==='snore'?1.5:kind==='crash'?.65:kind==='creak'?.48:.23,n=audioContext.sampleRate*duration,b=audioContext.createBuffer(1,n,audioContext.sampleRate),data=b.getChannelData(0);for(let i=0;i<n;i++)data[i]=Math.random()*2-1;
    const src=audioContext.createBufferSource();src.buffer=b;const f=audioContext.createBiquadFilter();f.type=kind==='crash'?'highpass':'bandpass';f.frequency.value=kind==='snore'?150:kind==='creak'?520:kind==='bed'?240:kind==='crash'?1700:120;f.Q.value=kind==='creak'?9:1.2;src.connect(f);f.connect(g);g.gain.setValueAtTime(.001,now);g.gain.exponentialRampToValueAtTime(loud,now+.035);g.gain.exponentialRampToValueAtTime(.001,now+duration);src.start();src.stop(now+duration);
  }else{
    const notes=kind==='win'?[392,494,587,784]:kind==='found'?[440,659]:kind==='lose'?[220,185,146]:kind==='notice'?[620,660]:[340,510];
    notes.forEach((hz,i)=>{const o=audioContext.createOscillator(),ng=audioContext.createGain();o.frequency.value=hz;o.connect(ng);ng.connect(g);ng.gain.setValueAtTime(0,now+i*.12);ng.gain.linearRampToValueAtTime(loud,now+i*.12+.02);ng.gain.exponentialRampToValueAtTime(.001,now+i*.12+.35);o.start(now+i*.12);o.stop(now+i*.12+.4);});g.gain.value=.7;
  }
}
...
function context(){if(game.hidden)return'C · 离开掩体';if(game.mode)return'';const d=game.doorNear(),s=game.spotNear(),c=game.coverNear();if(d)return`E · 推开${d.name}`;if(s)return`E · 搜索${s.name}`;if(c)return`C · 躲在${c.name}后`;return game.hasDevice?'带设备回到暖光卧室':view.mode==='firstPerson'?'WASD 移动 · 鼠标看向四周':'WASD 移动 · 点击小地图切换视角';}
function drawMap(target,level,player=null){
  const ctx=target.getContext('2d'),w=target.width,h=target.height,maxX=level===0?10:14,pad=12,unit=Math.min((w-pad*2)/(maxX+1),(h-pad*2)/15),ox=(w-(maxX+1)*unit)/2,oz=(h-15*unit)/2;
  ctx.clearRect(0,0,w,h);ctx.fillStyle='#101b2d';ctx.fillRect(0,0,w,h);
  for(let z=0;z<15;z++)for(let x=0;x<=maxX;x++){ctx.fillStyle=wall(x,z,level)?'#4b5c75':z>=11&&x<7?'#9f865d':'#253c53';ctx.fillRect(ox+x*unit,oz+z*unit,unit+.2,unit+.2);}
  for(const c of COVERS){if(c.x>maxX)continue;ctx.fillStyle='#749f96';ctx.fillRect(ox+(c.x+.15)*unit,oz+(c.z+.15)*unit,unit*.7,unit*.7);}
  for(const[x,z]of PRESETS[level].creaks){ctx.fillStyle='#b18a5f';ctx.fillRect(ox+(x+.2)*unit,oz+(z+.2)*unit,.6*unit,.6*unit);}
  if(player){const px=ox+(player.x+.5)*unit,pz=oz+(player.z+.5)*unit;ctx.save();ctx.translate(px,pz);ctx.rotate(view.mode==='firstPerson'?view.yaw:Math.PI-player.heading);ctx.fillStyle='#fff1c9';ctx.beginPath();ctx.moveTo(0,-6);ctx.lineTo(4,5);ctx.lineTo(0,3);ctx.lineTo(-4,5);ctx.closePath();ctx.fill();ctx.restore();}
}
...
function updateUI(){
  ...
  $('#look-hint').hidden=view.mode!=='firstPerson'||!game.active||!!document.pointerLockElement;drawMap($('#minimap'),game.level,game.player);
  const noise=Math.max(0,game.noise*(1-game.noiseAge/3));$('#noise-fill').style.width=`${noise}%`;$('#noise-label').textContent=game.noiseAge<2?noise>25?'刚才有点响':'轻轻的':'听一听';$('#detection').hidden=p.recognition<=0;$('#detection-fill').style.width=`${Math.min(100,p.recognition/1.65*100)}%`;
...
$('#context').onclick=()=>{if(game.hidden||game.coverNear()&&!game.doorNear()&&!game.spotNear())game.hide();else game.action();};
...
let last=performance.now(),fps=60,frames=0,acc=0,wallBlend=0,walkPhase=0;
function animate(now){
  ...
  const body=playerMesh.userData.body;body.position.y=game.hidden?-.30:moving?Math.abs(Math.sin(walkPhase))*.025:0;body.rotation.x=game.hidden?-.18:0;
  ...
  if(fp){const bob=settings.headBob&&moving?Math.sin(walkPhase)*.013:0;targetPos.set(game.player.x,game.hidden?.85:1.37+bob,game.player.z);targetRotation.setFromEuler(new THREE.Euler(view.pitch,-view.yaw,0,'YXZ'));}
  else{const center=game.level===0?5:7;targetPos.set(center+11,19,26);const m=new THREE.Matrix4().lookAt(targetPos,new THREE.Vector3(center,.3,7),new THREE.Vector3(0,1,0));targetRotation.setFromRotationMatrix(m);}
