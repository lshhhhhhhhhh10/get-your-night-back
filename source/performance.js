...
export function evaluatePerformance(game){
 ...
 const rows=[
  ...
  {name:'动作判断',max:25,points:Math.round(25*(actions?bounded(good/actions):1)),detail:`落脚 ${m.goodSteps}/${m.steps} · 接物 ${m.catches}/${m.incidents} · 推门${m.doorSeconds?Math.round(100*bounded(m.quietDoorSeconds/m.doorSeconds))+'% 安静':'尚未操作'}`},
