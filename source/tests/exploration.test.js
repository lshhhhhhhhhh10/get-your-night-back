...
test('扩建地图中全部候选藏点可由玩家抵达并原路返程',()=>{
  for(let level=0;level<3;level++){
    ...
    for(let i=0;i<queue.length;i++){const p=queue[i];for(const[dx,dz]of[[.5,0],[-.5,0],[0,.5],[0,-.5]]){const x=p.x+dx,z=p.z+dz,k=`${x},${z}`;if(!seen.has(k)&&!playerBlocked(x,z,level)){seen.add(k);queue.push({x,z});}}}
    for(const[x,z,name]of PRESETS[level].spots)assert.ok(queue.some(p=>distance(p,{x,z})<1.2),`${level}: ${name}`);
    assert.ok(queue.some(p=>p.x>14));assert.ok(queue.some(p=>p.z>14));
...
test('父母连续看见玩家四分之一秒就识别，预警仍保留五秒',()=>{
  ...
  const h=new Game(2);h.start();h.beginWarning();assert.equal(h.parent.timer,5);assert.deepEqual(h.inspectionTarget,{x:3,z:9});
});
