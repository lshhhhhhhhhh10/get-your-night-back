...
test('无效关卡与损坏 JSON 不会成为 Start 的续玩入口',()=>{const s=memory();for(const value of ['{broken',JSON.stringify({version:3,game:{level:999,status:'won',time:20}})]){s.setItem(SAVE_KEY,value);assert.equal(readSession(s),null);}});
test('设置限制范围，正常进度保留视角与关卡',()=>{const s=memory();s.setItem(SETTINGS_KEY,JSON.stringify({master:99,ambience:-3,sensitivity:8,headBob:true,vibration:true}));assert.deepEqual(readSettings(s),{master:1,effects:.7,music:.35,ambience:0,sensitivity:2,gamepadSensitivity:1,stickDeadzone:.16,headBob:true,vibration:true,hapticIntensity:.75,adaptiveTriggers:true,triggerStrength:.6});const g=new Game(1);g.start();saveSession(s,g,{yaw:.4,pitch:0,mode:'firstPerson',hasMoved:true});assert.equal(readSession(s).game.level,1);assert.equal(readSession(s).view.mode,'firstPerson');});
